...
object Linkifier {
  ...
  private def processText(text: String, regex: Regex, f: String ⇒ Frag): Seq[LinkifierNode] = {
    regex.findFirstMatchIn(text) match {
      case Some(rm) ⇒
        ...
        val texts = Seq(text.take(rm.start), text.drop(rm.end)).filter(_.nonEmpty).flatMap(processText(_, regex, f))
        (texts.take(1) :+ InlineDom(f(matched))) ++ texts.drop(1)
  ...
  def apply(text: String)(implicit ctx: Ctx.Owner): Seq[LinkifierNode] = {
    Seq(inlineYoutube _, inlineVideos _, linkify _).foldLeft(Seq[LinkifierNode](InlineText(text))) {
      case (nodes, f) ⇒
        nodes.flatMap {
